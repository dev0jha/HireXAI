...
export class AnalysisService {
   ...
   * **/
  public static async fetchGitHubRepo(
    owner: string,
    repo: string
  ): PromiseRes<RepoFetchResult> {
    ...
    switch (res.status) {
      ...
      case 302:
        return err(
          new Error(
            "Repository was moved (redirect). Use the canonical GitHub URL."
          )
        );

      case 400:
        return err(
          new Error("Bad request to GitHub. Check the owner/repo format.")
        );

      case 401:
        return err(
          new Error("Unauthorized GitHub request. Server token may be invalid.")
        );
      ...
      case 403: {
        ...
        if (rateLimit === "0") {
          ...
          return err(
            new Error(`GitHub rate limit exceeded. Try again after ${resetAt}.`)
          );
        }
      ...
      case 404:
        return err(
          new Error("Repo not found. Make sure it exists and is public.")
        );
      ...
      case 504:
        return err(
          new Error("GitHub is having server issues. Try again later.")
        );
  ...
  public static async *analyzeRepository({
    ...
    if (!validationRes.success) {
      yield sse({
        data: JSON.stringify({ error: validationRes.error.issues[0].message }),
      });
      return;
    }
    ...
    const analysisRes = await AnalysisService.analyze(
      owner,
      repo,
      repoData.data
    );
