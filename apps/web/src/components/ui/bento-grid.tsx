...
export const BentoGrid = ({ children, className, ...props }: BentoGridProps) => {
   return (
      <div
         className={cn("grid w-full grid-cols-1 gap-4 md:grid-cols-6 md:gap-4", className)}
         {...props}
      >
         {children}
      </div>
...
export const BentoCard = ({
   ...
   >
      ...
      <div className="relative h-full w-full overflow-hidden rounded-xl">
         ...
         <div className="absolute right-0 bottom-0 left-0 z-20 bg-linear-to-t from-[#121212] via-[#121212]/80 to-transparent p-6 pt-12">
            ...
            <div className="transform-gpu transition-all duration-300 group-hover:-translate-y-2">
               <h3 className="mb-1 text-lg font-semibold tracking-tight text-zinc-100">{name}</h3>
               <p className="line-clamp-2 text-xs leading-relaxed text-zinc-400">{description}</p>
            </div>
